...
def pos_setup(n):
    ...
    if n == 1:
        ...
        num_outer = int(np.floor(2*np.pi*radius / 3.25))
        num_inner = int(np.floor(2*np.pi*(radius-3) / 3.25))
        ...
        num_outer = 30 #int(np.floor(2*np.pi*radius / 3))
        ...
        inner_rotations = 2 * np.pi * np.array(range(num_inner)) / num_inner
        inner_pos = (radius - 3.75) * np.column_stack((np.cos(inner_rotations), np.zeros_like(inner_rotations), np.sin(inner_rotations)))
